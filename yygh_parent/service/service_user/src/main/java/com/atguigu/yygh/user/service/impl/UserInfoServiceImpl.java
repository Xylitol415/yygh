...
public class UserInfoServiceImpl extends ServiceImpl<UserInfoMapper, UserInfo>
    ...
    public Map<String, Object> loginUser(LoginVo loginVo) {
        ...
        if(userInfo == null) {
            // 判断是否第一次登录：根据手机号查询数据库，如果不存在手机号就是第一次登录
    ...
    private UserInfo packageUserInfo(UserInfo userInfo) {
        ...
        // 处理证件类型：10-身份证 20-户口本
        if(!StringUtils.isEmpty(userInfo.getCertificatesType())) {
            String certificatesType = Integer.parseInt(userInfo.getCertificatesType()) == 10 ? "身份证" : "户口本";
            userInfo.getParam().put("certificatesType",certificatesType);
        }
        return userInfo;
    }
}
