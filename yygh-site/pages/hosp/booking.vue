          ...
          <div>
                ...
                <div class="v-card clickable item ">
                  <div class="inline" v-for="(item,index) in patientList" :key="item.id" @click="selectPatient(index)" style="margin-right: 10px;">
                    ...
                    <div :class="activeIndex == index ? 'item-wrapper selected' : 'item-wrapper'">
                      ...
                      <img src="https://img.114yygh.com/static/web/checked.png" class="checked">
                    </div>
                  </div>
                </div>
              </div>
            ...
            <el-card class="patient-card" shadow="always" v-if="patientList.length > 0">
              ...
              <div class="card SELF_PAY_CARD">
                <div class="info"><span class="type">{{ patient.isInsure == 0 ? '自费' : '医保'}}</span><span class="card-no">{{ patient.certificatesNo }}</span><span class="card-view">居民身份证</span></div>
...
export default {
  ...
  methods: {
    ...
    submitOrder () {
      ...
      this.submitBnt = '正在提交...'
      orderInfoApi.submitOrder(this.scheduleId, this.hosScheduleId, this.patient.id).then(response => {
